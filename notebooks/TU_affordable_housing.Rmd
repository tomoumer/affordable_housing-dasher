...
sales_and_HUD <- sales_and_HUD %>% 
  mutate(group=case_when(
    ...
    YR_PIS-year(ownerdate) >= 2 &
    YR_PIS-year(ownerdate) <=5 ~ 'pre',
    as.numeric(nearest_HUD_dist) < 804.67 &
    YR_PIS-year(ownerdate) >= 0 &
    YR_PIS-year(ownerdate) <=2 ~ 'mid',
    as.numeric(nearest_HUD_dist) < 804.67 &
...
```{r}
sales_and_HUD <- sales_and_HUD %>% 
  mutate(age=year(ownerdate)-year_built)
```
...
> The intercept in this case doesn't make much sense. The square footage and year both have a positive inpact on increasing the price. Everything else seems to decrease the price. Inside the group in particular, pre and other in particular
